        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="relative rounded-xl">
              ...
              {unreadCount > 0 && <Badge className="absolute -top-1 -right-1 h-5 w-5 p-0 flex items-center justify-center text-xs bg-error-red text-white">
                  {unreadCount}
                </Badge>}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-80 backdrop-blur-sm border-border/50 rounded-2xl bg-zinc-900">
            ...
            <ScrollArea className="h-64">
              {notifications.length === 0 ? <div className="p-4 text-center text-muted-foreground text-sm">
                  No notifications
                </div> : notifications.map(notification => <DropdownMenuItem key={notification.id} className={`p-3 cursor-pointer rounded-xl ${!notification.read ? 'bg-primary/5' : ''}`} onClick={() => markAsRead(notification.id)}>
