...
const ChatConsole = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const responses = ['I understand you want to modify the deployment. Let me analyze the current workflow and suggest the best approach.', 'Creating a new workflow with your specifications. I\'ll integrate the necessary tools and configure them automatically.', 'I\'ve identified an issue in your pipeline. Here\'s a quick fix that should resolve it.', 'Your workflow is now optimized! I\'ve reduced deployment time by 35% and added automated rollback capabilities.'];
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {message.workflowId && <Badge variant="outline" className="text-xs">
                                {message.workflowId}
                              </Badge>}
                          </div>
          ...
          <Card className="gradient-card">
            ...
            <CardContent>
              <div className="space-y-2">
                {['React + Docker + K8s', 'Node.js Microservices', 'Python FastAPI Deploy', 'Static Site + CDN'].map(template => <Button key={template} variant="ghost" className="w-full justify-start text-sm" onClick={() => setWorkflowPrompt(`Use template: ${template}`)}>
                    {template}
                  </Button>)}
              </div>
            </CardContent>
